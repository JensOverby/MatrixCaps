...
def main():
    ...
    obj = ObjLoader()
    obj.load_model("res/Axes_tripod.obj", scale=550.)
    ...
    while not glfw.window_should_close(window):
        ...
        if make_samples_count > 0:
            ...
            rot = pyrr.Matrix44.from_x_rotation(0) #random.random()*2.0*math.pi)
            rot *= pyrr.Matrix44.from_y_rotation(0) #random.random()*2.0*math.pi)
            trans = pyrr.Matrix44.identity(float)
            trans[3,0] = random.random()*1.0 - 0.5
            trans[3,1] = random.random()*1.7 - 0.85
            ...
            glUniformMatrix4fv(light_loc, 1, GL_FALSE, right_eye_transform*mat)
