...
def CapsNetBasic(input_shape, n_class=2):
    ...
    #kaj = K.sum(x)
    #K.shape(kaj)
    #kaj = layers.Lambda(lambda x: x ** 2)(kaj)
    #kaj = layers.Reshape((1,))(kaj)
    #kaj = layers.Flatten()(x)

    #x_shape = K.shape(x)
    #tf.print(x_shape, output_stream=sys.stdout)
    #conv1 = tf.Print(conv1, [conv1], "kaaaaaaaaaaaaaaaaaaaaaaaaj: ") #, output_stream=sys.stdout)

    # Layer 1: Just a conventional Conv2D layer
    conv1 = layers.Conv2D(filters=256, kernel_size=5, strides=1, padding='same', activation='relu', name='conv1')(x)

    #conv1 = tf.Print(conv1, [conv1])
