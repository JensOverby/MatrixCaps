...
def get_error(yhat, ygt):
    ...
    xyzErr = linalg.norm(yhat[:,3:] - ygt[:,3:], ord=1, axis=1)
...
class geodesic_loss(nn.Module):
    ...
    def forward(self, ypred, ytrue):
        angle_pred = torch.norm(ypred[:,:3], 2, 1)
        angle_true = torch.norm(ytrue[:,:3], 2, 1)
        axis_pred = F.normalize(ypred[:,:3])
        axis_true = F.normalize(ytrue[:,:3])
        ...
        xyz_loss = self.mse_loss(ypred[:,3:], ytrue[:,3:])
        if self.reduce:
            return torch.mean(theta) #+ xyz_loss
        else:
            return theta #+ xyz_loss
