...
class SparseCoding(nn.Module):
    ...
    def __init__(self, active=True, ema_decay=0.99, steepness_factor=12, clip_threshold=0.01, target_min_boost=0.96, target_max_boost=3.84, boost_factor=0.01, boost_update_count=50):
    ...
    def forward(self, x):
        ...
        capsule_routing = x[1].max(dim=1)[0]
        capsule_routing = capsule_routing.sum(dim=(-2, -1)) # batch-size, output_dim
        ...
        _, order = capsule_routing.topk(capsule_routing.shape[1])
        _, ranks = (-order).topk(capsule_routing.shape[1]) # batch-size, output_dim
    
        """ Winning frequency """
        transposed_ranks = ranks.transpose(1,0)  # output_dim, batch_size
        ...
        normalised_ranks = ranks.float() / (ranks.shape[1] - 1) # batch_size, output_dim
    
        routing_mask = torch.exp(-self.steepness_factor * normalised_ranks)
        ...
        if self.N == self.boost_update_count:
            ...
            self.boosting_weights = self.boosting_weights.clamp(min=0.1, max=1.0)
