...
class PrimToCaps(nn.Module):
    def forward(self, x_orig):
        x = x_orig[0].permute(0, 1, 3, 4, 2).contiguous()                   # batch_size, input_dim, dim_x, dim_y, input_atoms
...
class MaxRouteReduce(nn.Module):
    ...
    def forward(self, x):
        # batch, output_dim, h, (dim_x, dim_y)

        """ Calculate routing coefficients """
        # route: batch, (input_dim), output_dim, dim_x, dim_y .... input_dim removed by MaxRoutePool
        #capsule_routing = x[1].max(dim=1)[0] # batch, output_dim, dim_x, dim_y
        #capsule_routing = x[1].sum(dim=1) # batch, output_dim, dim_x, dim_y
        capsule_routing = x[1].sum(dim=1) # batch, dim_x, dim_y

        """ Rank routing coefficients """
        x_sh = x[0].shape
        a_sort = capsule_routing.view(x_sh[0], -1).sort(1, descending=True)[1]
        a_sort = a_sort[:,None,None,:].repeat(1,x_sh[1],x_sh[2],1)

        x_sorted = x[0].view_as(a_sort).gather(3, a_sort)
        best = x_sorted[:,:,:,:self.out_sz]
        rest = x_sorted[:,:,:,self.out_sz:]
        idx_lucky = torch.randperm(rest.size(3))[:self.rnd_out_sz]
        x_sorted = torch.cat([best,rest[:,:,:,idx_lucky]], dim=3)

        idx = torch.randperm(self.out_sz+self.rnd_out_sz)
        x_sorted = x_sorted[:,:,:,idx].unsqueeze(-1)        # batch, output_dim, h, dim_x, dim_y

        x = x_sorted.permute(0, 1, 3, 4, 2).contiguous()    # batch_size, input_dim, dim_x, dim_y, input_atoms
        return (x.view(x.size(0), -1, x.size(-1), 1, 1))    # batch_size, input_dim*dim_x*dim_y, input_atoms, 1, 1
...
class MaxRoutePool(nn.Module):
    ...
    def forward(self, x):
        """
        x-value: batch_size, output_dim, h, dim_x, dim_y
        ...
        x0 = x[0]
        #x0[:,:,:2,:,:] = x0[:,:,:2,:,:] + self.add #* self.scale
        
        #routing = x[1].max(dim=1)[0] # batch, output_dim, dim_x, dim_y
        routing = x[1].sum(dim=1) # batch, output_dim, dim_x, dim_y
        _, indices = self.maxpool(routing)
        x_shp = x0.shape
        i_shp = indices.shape
        indices = indices.view(i_shp[0],i_shp[1],-1)
        x1 = routing.view(x_shp[0],x_shp[1],-1).gather(2, indices).view(i_shp)
        indices = indices[:,:,None,:].repeat(1,1,x_shp[2],1)
        x0 = x0.view(x_shp[0],x_shp[1],x_shp[2],-1).gather(3, indices).view(x_shp[0],x_shp[1],x_shp[2],i_shp[-1],i_shp[-1])
        return (x0, x1)
...
class CapsuleLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.type == 0:
            ...
            logit_shape = list(votes.size())
            logit_shape.pop(3)                                      # batch_size, input_dim, output_dim, dim_x, dim_y
            routed = dynamic_routing(votes=votes, biases=biases_replicated, logit_shape=logit_shape, num_routing=self.num_routing)
            """
            if self.do_sort:
